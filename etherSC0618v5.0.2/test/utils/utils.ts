...
export async function mintUtil(instance: Contract, admin: Signer, otherAccount: Signer, minter1: Signer) {
...
export async function mintRoleUtil(
  instance: Contract,
  admin: Signer,
  // otherAccount: Signer,
  masterminter: Signer,
...
export async function transferUtil(
  ...
  await mintRoleUtil(instance, admin, masterminter, minter1);
...
export async function transferFromUtil(
  instance: Contract,
  admin: Signer,
  otherAccount: Signer,
  masterminter: Signer,
  ...
  const otherBalance = await adminInstance.balanceOf(await otherAccount.getAddress());
  expect(otherBalance).to.equal(ethers.toBigInt("0"));
  ...
  await adminInstance.approve(masterminter, amount);

  // Connect masterminter to the instance
  const masterminterInstance = instance.connect(masterminter) as Contract;

  //transfer
  await masterminterInstance.transferFrom(admin, otherAccount, amount);

  return [adminBalance, otherBalance, amount];
}
...
export async function approveUtil(
  instance: Contract,
  admin: Signer,
  otherAccount: Signer,
  masterminter: Signer,
  ...
  // Connect admin to the instance
  const adminInstance = instance.connect(admin) as Contract;
  await adminInstance.approve(otherAccount, 21_000_000);
}

export async function removeMinterUtil(instance: Contract, masterminter: Signer, minter1: Signer) {
  expect(await instance.hasRole(await instance.MASTER_MINTER_ROLE(), masterminter)).to.equal(true);
  expect(await instance.hasRole(await instance.MINTER_ROLE(), minter1)).to.equal(true);
  ...
  //removeMinter
  await masterminterInstance.removeMinter(await minter1.getAddress());
}

export async function addFeeAccountUtil(instance: Contract, otherAccount: Signer, feemanager: Signer) {
  // Connect feemanager to the instance
  const feemanagerInstance = instance.connect(feemanager) as Contract;

  //addFeeAccount
  await feemanagerInstance.addFeeAccount(await otherAccount.getAddress());
}

export async function removeFeeAccountUtil(instance: Contract, feehold1: Signer, feemanager: Signer) {
  expect(await instance.hasRole(await instance.FEE_MANAGER_ROLE(), feemanager)).to.be.true;
  expect(await instance.hasRole(await instance.FEE_HOLDER_ROLE(), feehold1)).to.be.true;
  // Connect feemanager to the instance
  const feemanagerInstance = instance.connect(feemanager) as Contract;

  //removeFeeAccount
  await feemanagerInstance.removeFeeAccount(await feehold1.getAddress());
}

export async function updateFeeManagerUtil(instance: Contract, owner: Signer, otherAccount: Signer) {
  // Connect owner to the instance
  const ownerInstance = instance.connect(owner) as Contract;

  //updateFeeManager
  await ownerInstance.updateFeeManager(await otherAccount.getAddress());
}

export async function updateWhitestatusUtil(instance: Contract, whitelistManager: Signer, status: Boolean) {
  // Connect whitelistManager to the instance
  const whitelistManagerInstance = instance.connect(whitelistManager) as Contract;

  //updateWhitestatus
  await whitelistManagerInstance.updateWhitestatus(status);
}
