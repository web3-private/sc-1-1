...
describe("TokenUpgradeable contract", function () {
  ...
  describe("Roles and Permissions", function () {
    it("Should grant and check ADMIN_ROLE", async function () {
      ...
      console.log("owner:::", await deployInstance.owner());
      ...
      const instance = (await deployInstance.connect(owner)) as Contract;

      // Grant ADMIN_ROLE to otherAccount
      await instance.grantRole(ADMIN_ROLE, otherAccount.address);
      // await deployInstance.grantRole(ADMIN_ROLE, otherAccount.address);

      // Check if otherAccount has ADMIN_ROLE
      expect(await instance.hasRole(ADMIN_ROLE, otherAccount.address)).to.be.true;
      // expect(await deployInstance.hasRole(ADMIN_ROLE, otherAccount.address)).to.be.true;
    });

    it("Should grant and check PAUSER_ROLE", async function () {
      const { deployInstance, owner, admin, otherAccount } = await loadFixture(deployTokenUpgradeableFixture);
      // const { instance, initialOwner, otherAccount } = await loadFixture(deployTokenUpgradeableFixture);
      ...
      // await deployInstance.grantRole(PAUSER_ROLE, otherAccount.address);

      // Check if otherAccount has PAUSER_ROLE
      expect(await instance.hasRole(PAUSER_ROLE, otherAccount.address)).to.be.true;
      // expect(await deployInstance.hasRole(PAUSER_ROLE, otherAccount.address)).to.be.true;
    });
    ...
    it("Should grant and check BLACKLISTER_ROLE", async function () {
      ...
      // await deployInstance.grantRole(BLACKLISTER_ROLE, otherAccount.address);

      // Check if otherAccount has BLACKLISTER_ROLE
      expect(await instance.hasRole(BLACKLISTER_ROLE, otherAccount.address)).to.be.true;
      // expect(await deployInstance.hasRole(BLACKLISTER_ROLE, otherAccount.address)).to.be.true;
    });
    ...
    it("Should grant and check WHITELISTER_ROLE", async function () {
      ...
      // await deployInstance.grantRole(WHITELISTER_ROLE, otherAccount.address);

      // Check if otherAccount has WHITELISTER_ROLE
      expect(await instance.hasRole(WHITELISTER_ROLE, otherAccount.address)).to.be.true;
      // expect(await deployInstance.hasRole(WHITELISTER_ROLE, otherAccount.address)).to.be.true;
    });

    it("Should grant and check MASTER_MINTER_ROLE", async function () {
      const { deployInstance, owner, admin, otherAccount } = await loadFixture(deployTokenUpgradeableFixture);
      // const { instance, initialOwner, otherAccount } = await loadFixture(deployTokenBaseUpgradeableFixture);
      ...
      // await deployInstance.grantRole(MASTER_MINTER_ROLE, otherAccount.address);
      ...
      // expect(await deployInstance.hasRole(MASTER_MINTER_ROLE, otherAccount.address)).to.be.true;
    });
    ...
    it("Should grant and check MINTER_ROLE", async function () {
      ...
      // await deployInstance.grantRole(MINTER_ROLE, otherAccount.address);

      // Check if otherAccount has MINTER_ROLE
      expect(await instance.hasRole(MINTER_ROLE, otherAccount.address)).to.be.true;
      // expect(await deployInstance.hasRole(MINTER_ROLE, otherAccount.address)).to.be.true;
    });
    ...
    it("Should grant and check FEE_MANAGER_ROLE", async function () {
      ...
      // await deployInstance.grantRole(FEE_MANAGER_ROLE, otherAccount.address);

      // Check if otherAccount has FEE_MANAGER_ROLE
      expect(await instance.hasRole(FEE_MANAGER_ROLE, otherAccount.address)).to.be.true;
      // expect(await deployInstance.hasRole(FEE_MANAGER_ROLE, otherAccount.address)).to.be.true;
    });
    ...
    it("Should grant and check FEE_HOLDER_ROLE", async function () {
      ...
      // await deployInstance.grantRole(FEE_HOLDER_ROLE, otherAccount.address);

      // Check if otherAccount has FEE_HOLDER_ROLE
      expect(await instance.hasRole(FEE_HOLDER_ROLE, otherAccount.address)).to.be.true;
      // expect(await deployInstance.hasRole(FEE_HOLDER_ROLE, otherAccount.address)).to.be.true;
    });
    ...
    it("Should allow only PAUSER_ROLE to pause the contract", async function () {
      ...
      // const instance = await deployInstance.connect(otherAccount) as Contract;
      await instance.pause();
      expect(await instance.paused()).to.be.true;

      // Unpause the contract with PAUSER_ROLE
      // await instance.connect(otherAccount).unpause();
    ...
    it("Should allow only BLACKLISTER_ROLE to blacklist accounts", async function () {
      ...
      // await instance.connect(otherAccount).blacklist(initialOwner.address);
      // const instance = await deployInstance.connect(otherAccount) as Contract;
      // instance.blacklist(initialOwner.address);
      ...
      // await instance.connect(otherAccount).unBlacklist(initialOwner.address);
