...
import {TokenBaseUpgradeable0716} from "./TokenBaseUpgradeable0716.sol";
import {Whitelistable0716} from "./Whitelistable0716.sol";

import {Math} from "@openzeppelin/contracts/utils/math/Math.sol";
import {SignatureChecker} from "@openzeppelin/contracts/utils/cryptography/SignatureChecker.sol";

contract TokenUpgradeable0716 is TokenBaseUpgradeable0716, Whitelistable0716 {
    ...
    {
        require(
            block.timestamp >= lastLockTime[_msgSender()] + MINT_DELAY,
            "TokenUpgradeable: minting not allowed yet"
        );
        ...
        // if(to == owner() && _msgSender() != owner()) {
        //     revert();
        //     // revert("msg.sender address is not equal to the owner address");
        // }
        if (to == owner()) {
            require(_msgSender() == owner());
        }
        // if(to == owner()) {
        //     require(_msgSender() == owner(), "TokenUpgradeable: msg.sender address is not equal to the owner address");
        // }

        super._mint(to, amount);
        lastLockTime[_msgSender()] = block.timestamp;
