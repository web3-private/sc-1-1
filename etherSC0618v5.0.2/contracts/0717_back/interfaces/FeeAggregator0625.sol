...
import {FeeV1Interface0625} from "./FeeV1Interface0625.sol";

import {TransparentUpgradeableProxy} from "@openzeppelin/contracts/proxy/transparent/TransparentUpgradeableProxy.sol";

contract FeeAggregator0625 is FeeV1Interface0625, OwnableUpgradeable {
    ...
    function updateFeeAccountAmount(address account, uint256 amount) external override onlyOwner returns (bool) {
    ...
    function _setCommonParam(
        ...
        (, uint256 fee) = newMaxFee.tryMul(uint(10) ** _decimals);
        maximumFee = fee;
    ...
    function calcCommonFee(uint256 _value) public view override returns (uint256) {
        ...
        // require(basicFlag, "Multiplication overflow");

        (bool flag, uint256 fee) = basicFee.tryDiv(1000);
        if (!flag) {
            revert DivisionOverflow();
        }
        // require(flag, "Division overflow");
        // (bool flag, uint256 fee) = (_value.tryMul(basisPointsRate)).tryDiv(1000);
