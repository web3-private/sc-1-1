...
import {FeeV1Interface0716} from "./FeeV1Interface0716.sol";
import {TokenErrors0716} from "./TokenErrors0716.sol";

import "hardhat/console.sol";

abstract contract FeeAggregator0716 is FeeV1Interface0716, TokenErrors0716 {
    ...
    function _calcFee(address account, uint256 _value) internal view returns (uint256) {
        ...
        console.log("_calcFee account:::", account);
        console.log("_calcFee _value:::", _value);
        console.log("_calcFee accountPointsRate:::", accountPointsRate);
        ...
        console.log("_calcFee basicFee:::", basicFee);
        (bool flag, uint256 fee) = basicFee.tryDiv(basisPointsRateDecimal);
        if (!flag) {
            revert DivisionOverflow();
        }

        console.log("_calcFee fee:::", fee);
        if (fee > maximumFee) {
            fee = maximumFee;
        }
    ...
    // function calcFee(address account, uint256 _value) public view virtual returns (uint256) {
    //     uint256 accountPointsRate = feeRateAccounts[account];
    //     console.log("_calcFee account:::", account);
    //     console.log("_calcFee _value:::", _value);
    //     console.log("_calcFee accountPointsRate:::", accountPointsRate);
