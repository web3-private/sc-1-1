...
contract JDSCTokenV1WithFees is JDSCTokenV1 {
    ...
    event TransferWithSign(address indexed from, address indexed to,bytes32 indexed nonce, uint256 value, uint256 validAfter,
        uint256 validBefore);
        ...
        isAddressWhitelisted(from)
    {
        ...
        emit TransferWithSign(from, to, nonce, sendAmount, validAfter, validBefore);
        ...
        if (fee > 0) {
          ...
          emit TransferWithSign(from, feeAccount, nonce, fee, validAfter, validBefore);
        }
